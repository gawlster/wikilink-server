...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        res.status(500).json({ message: "Failed to retrieve completed game" });
        return;
    }
    ...
    if (completedGame.createdFromSeed) {
        ...
        try {
            ...
            if (!seededGame) {
                ...
                res.status(404).json({ message: "Seeded game not found" });
                return;
            }
            return res.status(200).json(seededGame);
        } catch (error) {
            console.error("Error retrieving seeded game from completed game:", error);
            res.status(500).json({ message: "Failed to retrieve seeded game from completed game" });
            return;
        }
    }
    try {
        ...
        res.status(500).json({ message: "Failed to create seeded game from completed game" });
    }
}
